...
Что, если ведется разработка большого приложения со сложным интерфейсом? Используя только данный синтаксиc, то разработка сильно замедлилась, так как он слишком многословен. Поэтому компания Facebook представила синтаксис JSX (*JavaScript Syntax Extension*), который сильно напоминает язык разметки HTML, но на самом деле является расширением JavaScript.
